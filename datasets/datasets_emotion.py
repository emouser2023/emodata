...
class Emotion_DATASETS(data.Dataset):
    def __init__(self, list_file,
                 num_segments=1, new_length=1, transform=None,
                 random_shift=True, test_mode=False, index_bias=1, root_path='',json_path='', config=None, sample_rate=16000):
    ...
    def get(self, record):
        ...
        video_path = os.path.join(self.root_path, record['video_clip'])
        
        # print(video_path)
        # exit(0)
        frames, indices, num_of_frames = self._load_video(video_path)
        audio_path = os.path.join(self.root_path,record['audio_clip'])
        ...
        label_str = record['final_emotion'].strip().lower()  
        label_idx = EMOTION_LABELS[label_str]
        ...
        return {'vidio':process_video, 'face_mask':face_mask_binary , 'human_mask':human_mask_binary,
                 'text':text, 'text_logits':text_logits, 'audio_logits':audio_logits, 'waveform':waveform,'label_id':label}
