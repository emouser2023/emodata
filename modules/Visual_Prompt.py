...
class MultimodalFusionClassifier(nn.Module):
    def __init__(self, embed_dim=512, num_classes=10, hidden_dim=512):
        super(MultimodalFusionClassifier, self).__init__()
        
        # input dimension = concat of 3 modalities
        input_dim = embed_dim * 3
        
        self.fc1 = nn.Linear(input_dim, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, num_classes)

    def forward(self, image_embed, text_embed, audio_embed):
        orig_dtype = image_embed.dtype
        # cast to float32 to match model params
        image_embed_exp = image_embed.to(torch.float32)
        text_embed_exp = text_embed.to(torch.float32)
        audio_embed_exp = audio_embed.to(torch.float32)
        
        fused = torch.cat([image_embed_exp, text_embed_exp, audio_embed_exp], dim=-1)  # [batch, 512*3]
    ...
    #     fused = torch.cat([image_embed, text_embed, audio_embed], dim=-1)  # shape: [batch, 512*3]
